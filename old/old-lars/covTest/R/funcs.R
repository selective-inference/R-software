...
SMALL=1e-6
if(max(abs(colMeans(x)))>SMALL  | max(apply(x,2,var)-1)>SMALL){
     stop("Have to center and scale x before calling lars.(glm)  and covTest;
use  mx=colMeans(x); sx=sqrt(apply(x,2,var)); x=scale(x,mx,sx)")
   }
errflag=0
 junk=unlist(lapply(fitobj$act,length))
     if(max(junk)>1){
         cat("multiple predictors entering at one knot",fill=T)
         errflag=1
         return(list(errflag=errflag))
}
...
#NOTE this is only for Gaussian for now and for sigma known
if(type=="lar"){
d=lamlist[-length(lamlist)]*(-diff(lamlist))/sigma^2
w=tt/d[1:length(tt)]
jonpv=jonf(lamlist,w,sigma,p)
}
...
if(type=="lar" & sigma.type=="known") {
out=cbind(out,jonpv[1:nrow(out)]);
        dimnames(out)[[2]]= c("Predictor_Number","Drop_in_covariance","P-value", "Exact P-value")
                   }
return(list(errflag=errflag,results=round(out,4),sigma=round(sigma,4),null.dist=null.dist,w=w))
}

lars.glm=function(x,y,status=NULL,family=c("binomial","cox"),standardize=TRUE,frac.arclength=.1,max.steps=10 * min(nrow(x), ncol(x))){
#
...
n=nrow(x)
if(family=="binomial")  {
      a=glmpath(x,y,family="binomial",lambda2=0,standardize=standardize,frac.arclength=frac.arclength,max.steps=max.steps)
      b=a$b.pred[,-1,drop=F]
   }
if(family=="cox") {
   d=list(x=x,time=y,status=status)
#here i decrease arclength a few times, to try to get at most one var entering each time
    a=coxpath(d,lambda2=0,frac.arclength=1,max.steps=max.steps)
    junk=unlist(lapply(a$act,length))
    if(max(junk)>1){ 
       a=coxpath(d,lambda2=0,frac.arclength=.5, max.steps=max.steps)
        junk=unlist(lapply(a$act,length))
     }
    if(max(junk)>1){
       a=coxpath(d,lambda2=0,frac.arclength=.1, max.steps=max.steps)
...
jonf=function(lamlist,w,sigma,p){
ww=sqrt(w)
# compute's new pvalue
jon=rep(NA,p)
for(j in 1:p){
if(j==1) p1=1
if(j>1) p1=pnorm(lamlist[j-1]/(sigma*ww[j]))
 p2=pnorm(lamlist[j]/(sigma*ww[j]))
 p3=pnorm(lamlist[j+1]/(sigma*ww[j]))
 jon[j]=(p1-p2)/(p1-p3)
}
return(jon)
}
