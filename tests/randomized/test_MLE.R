...
simple_problem = function(target_observed=2, threshold=2, randomization_scale=1.) {
    # Simple problem: randomizaiton of sd 1 and thresholded at 2 (default args)

    target_observed = as.numeric(target_observed)
    target_transform = list(linear_term=-diag(1), offset=c(0.))
    opt_transform = list(linear_term=diag(1), offset=c(threshold*1.))
    feasible_point = c(1.)
